...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
   ...
   /* private void converterobj(HttpServletRequest request, HttpServletResponse response) throws Exception{
    	
    	Gson gson;
    	List<Object>listobjConvert = new ArrayList();
    	Collection<Cliente> listObj = this.service.listacliente();
    	
    	for(Cliente c : listObj){
    			gson = new Gson();
    			gson.toJson(c);
    			listobjConvert.add(gson);
    				
    	}
    	RequestDispatcher rd =  request.getRequestDispatcher(Constantes.USUARIOS);
    	request.setAttribute("listobjConvert", listobjConvert);
    	rd.forward(request, response);
